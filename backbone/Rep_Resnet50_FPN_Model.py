...
class RepVGG_Bottleneck(nn.Module):
    expansion = 4
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, norm_layer=None):
        ...
        # -----------------------------------------
        self.conv2 = RepVGGBlock(in_channels=out_channel, out_channels=out_channel, kernel_size=3, stride=stride, padding=1)
...
class RepVGGBlock_ResNet(nn.Module):
...
def RepVGG_resnet50_fpn_backbone(pretrain_path="",
                          norm_layer=nn.BatchNorm2d,
                          trainable_layers=3,
    ...
    Args:
        pretrain_path:预训练权重，如果不使用就默认为空
        norm_layer: 默认是nn.BatchNorm2d
    ...
    """
    resnet_backbone = RepVGGBlock_ResNet(RepVGG_Bottleneck, [3, 4, 6, 3],
                             include_top=False,
