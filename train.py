...
from backbone import resnet50_fpn_backbone, RepVGG_resnet50_fpn_backbone, repvgg_model_convert
...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    backbone = RepVGG_resnet50_fpn_backbone(pretrain_path="", trainable_layers=5)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        model_val = repvgg_model_convert(model)
        det_info, seg_info = utils.evaluate(model_val, val_data_loader, device=device)
        new_det_info = det_info[0]
        new_seg_info = seg_info[0]
        ...
        if new_det_info > old_det_info:
            torch.save(save_files, "./save_weights/best_det_model.pth")
        if new_seg_info > old_seg_info:
            torch.save(save_files, "./save_weights/best_seg_model.pth")
        old_det_info = new_det_info
        old_seg_info = new_seg_info
        # save
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='data/UIIS/UDW', help='dataset')
    # 检测目标类别数(不包含背景)
    ...
    parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                        help='momentum')
    # SGD的weight_decay参数
    parser.add_argument('--wd', '--weight-decay', default=2e-3, type=float,
